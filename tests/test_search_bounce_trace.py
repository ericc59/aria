...
def test_search_programs_solves_142ca369_with_diagonal_collision_trace():
    ds = get_dataset("v2-eval")
    task = load_arc_task(ds, "142ca369")
    demos = [(pair.input, pair.output) for pair in task.train]

    program = search_programs(demos, time_budget=5.0)

    assert program is not None
    assert "diagonal_collision_trace" in program.description
