...
def search_program(
    ...
    *depth_preferred_fn*: callable(depth, max_steps) → frozenset[str]
    ...
    # --- A2 pruning: structural analysis ---
    same_shape = all(d.input.shape == d.output.shape for d in demos)
    # Ops that always change grid dimensions — useless for same-shape tasks
    _DIMS_CHANGE_OPS = frozenset({
        "dims_make", "new_grid", "crop", "crop_to_content", "crop_frame_interior",
        "crop_to_object", "get_row", "get_col", "stack_h", "stack_v",
        "tile_grid", "upscale_grid", "unique_rows", "unique_cols",
        "peel_frame", "select_and_separate",
    })
    ...
    for depth in range(1, max_steps + 1):
        ...
        # Merge same-shape exclusions into excluded ops
        depth_excluded = excluded_ops or frozenset()
        if same_shape:
            depth_excluded = depth_excluded | _DIMS_CHANGE_OPS
...
def _combo_cap(arity: int) -> int:
    ...
    if arity == 3:
        return 48
    # Higher arity: tighter cap to avoid explosion
    return 24
...
def _depth_candidate_budget(
    *,
    ...
    # Give depth 1 a small share (20%), depth 2+ gets the rest.
    # Depth 1 programs are cheap to enumerate — most solutions are depth 2+.
    if depth == 1:
        return max(1, remaining_budget // 5)
...
def _is_searchable_sig(name: str, sig: OpSignature) -> bool:
    if len(sig.params) == 0 or len(sig.params) > 5:
        return False
